...
function dailyHtml(task, file) {
  ...
  return `<section id="day-${task.day}" class="day-plan-card"><div class="eyebrow">Day ${task.day} of 5 · ${DAY_NAMES[task.day]} · ${escapeHtml(task.id)}</div><h2>${DAY_NAMES[task.day]} — ${escapeHtml(task.title)}</h2><div class="day-status-row"><span>Task status</span><strong class="status">${escapeHtml(task.status)}</strong></div><div class="day-boundary"><p><strong>Start only after:</strong> ${escapeHtml(startSummary(task))}</p><p><strong>Finish only when:</strong> ${escapeHtml(endResult(task))}</p></div><p><strong>Today's goal:</strong> ${escapeHtml(task.outcome)}</p><p><strong>Why today:</strong> ${escapeHtml(task.unlocks[0] || task.why)}</p><h3>Before you start</h3>${prerequisites}${task.integrationGates.length ? `<p><strong>Later integration or acceptance gate:</strong> ${escapeHtml(task.integrationGates.join(", "))}. This does not block today's approved construction scope.</p>` : ""}<h3>Build in this order</h3><ol>${scope}</ol><h3>Technical rules</h3>${rules ? `<ul>${rules}</ul>` : "<p>Follow the linked flow and contract without expanding scope.</p>"}<p><strong>Required references:</strong> ${escapeHtml(dailyReferences(task).join(", ") || "Canonical task source")}</p><h3>Tests for today</h3><ul>${tests}</ul><h3>End-of-day result</h3><p>${escapeHtml(endResult(task))}</p><p><strong>Recommended commit:</strong> <code>${escapeHtml(task.commit)}</code></p><p>The complete agent instructions for this day are embedded in the downloadable weekly Markdown.</p><nav class="day-navigation" aria-label="Day ${task.day} navigation"><a href="#five-day-map">Back to five-day map</a>${next}</nav></section>`;
}
...
function teamWeekHtml(file, week, prefix, tasks, options = {}) {
  ...
  const dayJump = `<nav class="week-day-jump" aria-label="${escapeHtml(label)} day navigation"><strong>Jump to</strong><a href="#five-day-map">Overview</a>${selected.map((task) => `<a href="#day-${task.day}"><span>Day ${task.day}</span> ${DAY_NAMES[task.day].slice(0, 3)}</a>`).join("")}<a href="#week-complete">Complete</a></nav>`;
  const body = [
